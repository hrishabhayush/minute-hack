...
import PolicyCard from '@/components/passenger/PolicyCard';
...
const generateMockPolicies = () => {
  const statuses = ['active', 'paid', 'expired'] as const;
  
  return Array(5).fill(null).map((_, i) => {
    const status = statuses[i % 3] as 'active' | 'paid' | 'expired';
    ...
    return {
      ...
      coverage: 50,
      status,
      delayMinutes: status === 'paid' ? 42 + i : undefined,
      payout: status === 'paid' ? 50 : undefined
    };
  });
};
